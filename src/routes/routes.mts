...
router
    .get("/temperature-stream", async (ctx) => {
        ...
        setInterval(async () => {
            ...
            let data: {
                data: { temperature: number, humidity: number, time_of_collection: string }[],
                error: string[]
            } = { data: [], error: [] }
            ...
            try {
                ...
                if (res && Array.isArray(res)) {
                    const dbRes = res as Array<{ temperature: number, humidity: number, 'time_of_collection': string }>
    ...
    .get("/history", async (ctx) => {
        ...
        try {
            ...
            // Daten zum Datenobjekt hinzufügen und das Datum format von SQL zu einem besser lesbarem ändern
            if (res && Array.isArray(res)) {
                data.data = res as Array<{ temperature: number, humidity: number, time_of_collection: string }>
            }
