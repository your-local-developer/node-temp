...
router.get("/temperature-stream", async (ctx) => {
    ...
    // Jede Sekunde die Temperatur lesen und als Objekt die gerundeten Werte dem Browser übersenden
    setInterval(async () => {
        const { temperature, humidity } = await readDhtSensor(11, 7);
        temperatureStream.write(`data: ${JSON.stringify({ temperature: temperature.toFixed(2), humidity: humidity.toFixed(2) })}\n\n`);
    }, 1000);
});
